...
describe('Game - 状態遷移・不整合データテスト', () => {
  ...
  describe('🔥 状態遷移の境界条件テスト', () => {
    it('未開始ゲームでの操作エラー', () => {
      ...
      // 未開始状態での不正操作
      expect(() => game.drawCardsSync(5)).toThrow()
    ...
    it('ゲームオーバー後の操作制限', () => {
      ...
      // ゲームオーバー後の操作が適切に制限されるか
      expect(() => game.drawCardsSync(1)).toThrow()
      expect(() => game.nextTurn()).toThrow()
    })
  ...
  describe('💀 キャッシュとダーティフラグの整合性', () => {
    it('活力変更時のキャッシュ無効化', () => {
      ...
      expect(available3).toBe(70) // 80 - 10(保険料)
    })
  ...
  describe('🎯 エッジケース・境界条件の組み合わせ', () => {
    it('活力0・保険料負担ありの状態', () => {
      ...
      expect(available).toBe(-10) // 負の値
      
      // ゲームオーバー状態か
      expect(game.isGameOver()).toBe(true)
    })

    it('最大手札数オーバーでのカードドロー', () => {
      game.start()
