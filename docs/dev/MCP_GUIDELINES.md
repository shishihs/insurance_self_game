...
### 1. デプロイ完遂の原則
- **常にコミットし、GitHub Pages へのデプロイが完了するところまでやり切ること**
- ローカルでの修正だけで満足せず、本番環境（GitHub Pages）での動作確認をもって作業完了とする

### 2. バージョン管理と可視化
- **デプロイごとにゲームのバージョンをインクリメントすること**
- `src/App.vue` 内の `console.log` に出力されるバージョン番号（例: `v2025.12.06.xxxx`）を更新し、デプロイごとの変化を追跡可能にする

### 3. マインドセット
- **高年収のシニアエンジニアとして振る舞うこと**
- 常にプロフェッショナルとしての品質、保守性、拡張性を意識したコードを書く
- 問題の対症療法的な修正だけでなく、根本原因の解決と将来的なリスクの排除を考える
- ユーザー（依頼者）に対して、技術的な観点からの付加価値を提供する提案を行う
