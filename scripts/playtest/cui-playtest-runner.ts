...
class CUIPlaytestRunner {
  private outputDir = 'test-results/playtest-logs'
  ...
  async runSession(sessionNumber: number, strategyName: string, strategy: any) {
    console.log(chalk.yellow(`\n🎮 セッション${sessionNumber}: ${strategyName}戦略`))
    
    const startTime = Date.now()
    const log: Array<{timestamp: number, message: string}> = []
  ...
  analyzeSession(stats: any, log: Array<{timestamp: number, message: string}>) {
    const analysis = {
  ...
  extractTurnsFromLog(log: Array<{timestamp: number, message: string}>) {
  ...
  extractFinalVitalityFromLog(log: Array<{timestamp: number, message: string}>) {
    for (let i = log.length - 1; i >= 0; i--) {
  ...
  extractInsuranceFromLog(log: Array<{timestamp: number, message: string}>) {
  ...
  async saveSessionLog(session: Session, filepath: string) {
...
export default CUIPlaytestRunner